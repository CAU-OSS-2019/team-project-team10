function showLane(tmpList) {
    // var sx = 126.93737555322481;
    // var sy = 37.55525165729346;
    // var ex = 126.88265238619182;
    // var ey = 37.481440035175375;
    var startList = [{'position': {'x': 126.961314, 'y': 37.4763176}}, {
        'position': {
            'x': 126.927075,
            'y': 37.487426
        }
    }, {'position': {'x': 126.990487, 'y': 37.532527}}];
        ...
        fetch(`http://165.194.35.214:26656/lane/loadLane/?mapObj=${mapObj}`)
            .then(function (res) {
                res.json().then(body => {
                    ...
                    drawNaverPolyLine(resultJsonData, pathArray, startPos, endPos);      // 노선그래픽데이터 지도위 표시
...
function drawNaverPolyLine(data, pathArray, startPos, endPos) {
    var lineArray;
    // console.log(data);
    ...
    for (var i = 0; i < data.result.lane.length; i++) {
        ...
        // if (i == 0){
        //    var walkPolylineArr = [new naver.maps.LatLng(startPos.position.y, startPos.position.x), lineArray[0]];
        ...
        //   var walkPolylineArr = [lineArray[lineArray.length-1], new naver.maps.LatLng(endPos.position.y, endPos.position.x)]
        //   var walkPolyline = new naver.maps.Polyline({
        //       map:map,
        //       path: walkPolylineArr,
        //       strokeWeight:10
...
function showPubInfo(pubInfo, position) {
    ...
    var point = new naver.maps.Point(position.x, position.y); // 좌표
